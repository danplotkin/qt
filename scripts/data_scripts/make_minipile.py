...
FLATTENED_CORPA_DIR = "data/flattened_corpa/minipile"
os.makedirs(FLATTENED_CORPA_DIR, exist_ok=True)


def get_minipile() -> DatasetDict:
    if not os.path.exists('data/minipile'):
        logger.info("Loading MiniPile dataset...")
        ds = load_dataset("JeanKaddour/minipile")
        ds.save_to_disk('data/minipile')
        logger.info("MiniPile dataset downloaded and saved to disk.")
    logger.info("MiniPile dataset loaded from disk.")
